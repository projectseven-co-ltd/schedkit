// NocoDB REST client — used when DATABASE_URL is not set.
...
export const db = {
  ...
  find: (tableId, where, params = {}) => {
    const qs = new URLSearchParams({ where, ...params }).toString();
    return req('GET', `/api/v1/db/data/noco/${BASE_ID}/${tableId}?${qs}`);
  },
};
