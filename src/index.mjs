...
await fastify.register(swagger, {
  ...
  openapi: {
    ...
    tags: [
      { name: 'Event Types', description: 'Create and manage bookable event types. Each event type has a slug used in the public manifest link (`/assign/:username/:slug`). Set `requires_confirmation: true` to hold assignments as pending until you manually accept or decline.' },
      { name: 'Availability', description: 'Define your weekly recurring availability. Rules are per day-of-week with a start/end time in HH:MM format. The slots engine uses these rules minus existing assignments and blocked times.' },
      ...
      { name: 'Blackout Dates', description: 'Block specific dates or date ranges from accepting assignments.' },
      ...
      { name: 'Notifications', description: 'Configure ntfy.sh push notification topics for new assignments.' },
